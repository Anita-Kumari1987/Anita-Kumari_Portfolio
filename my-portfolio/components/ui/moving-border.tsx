...
export const MovingBorder = ({
  ...
  // ✅ initialize ref with null and type it to an SVGGeometryElement
  ...
  const x = useTransform(progress, (val) => {
    ...
    return p ? p.getPointAtLength(val).x : 0;
  });

  const y = useTransform(progress, (val) => {
    const p = pathRef.current;
    return p ? p.getPointAtLength(val).y : 0;
  });
  ...
  return (
    <>
      ...
      >
        <rect
          ref={pathRef as React.RefObject<SVGRectElement>} // SVGRectElement extends SVGGeometryElement
          fill="none"
          width="100%"
          height="100%"
          rx={rx}
