...
export default function FAQJaydenDark() {
  ...
  <section className="relative overflow-hidden flex flex-col items-center justify-center rounded-3xl max-[550px]:rounded-2xl min-h-screen w-[92%] mx-auto px-4 md:px-2 py-12 md:py-8 isolate max-[550px]:pt-2">
      ...
      <div className="relative mx-auto max-w-5xl px-4 md:px-6 py-12 md:py-20 z-10">
        <div className="mb-8 md:mb-12 text-center relative">
          <h2 className="bg-clip-text text-center text-4xl md:text-[72px] leading-tight md:leading-none font-rajdhani tracking-tight text-transparent bg-gradient-to-b from-orange-200 to-orange-500">
            FAQs
          </h2>
          ...
          <div className="absolute -top-6 md:-top-10 -right-6 md:-right-10 w-20 h-20 md:w-32 md:h-32 bg-orange-500/10 rounded-full blur-2xl md:blur-3xl" />
          <div className="absolute -bottom-6 md:-bottom-8 -left-6 md:-left-8 w-16 h-16 md:w-24 md:h-24 bg-orange-600/10 rounded-full blur-xl md:blur-2xl" />
        </div>
          ...
          <div key={item.id} role="listitem">
            <h3>
              <button
                ...
                className="group flex w-full flex-col text-left rounded-2xl border border-white/10 bg-white/[0.04] p-4 md:p-6 backdrop-blur-xl transition-all duration-300 hover:border-white/20 hover:bg-white/[0.06]"
              >
                <div className="flex items-center justify-between w-full gap-3">
                  <span className="flex-1">
                    <TextAnimate
                      ...
                      once
                      delay={0.02}
                      className="text-xl md:text-2xl max-[550px]:text-base font-light text-white/90 group-hover:text-white"
                    >
                      {item.question}
                    </TextAnimate>
                  </span>
                  ...
                  <span
                    aria-hidden="true"
                    className={`grid h-9 w-9 md:h-10 md:w-10 shrink-0 place-items-center rounded-full border transition-all duration-300 ${
                      isOpen
                ...
                <div
                  ...
                  className={`overflow-hidden transition-all duration-300 ease-out ${
                    isOpen
                      ? "mt-4 md:mt-6 max-h-[420px] md:max-h-[500px] opacity-100"
                      : "max-h-0 opacity-0"
                  }`}
                >
                  <TextAnimate
                    ...
                    delay={0.012}
                    duration={0.6}
                    className="text-[15px] md:text-base leading-relaxed text-white/70 border-t border-white/10 pt-5 md:pt-6"
                  >
                    {item.answer}
                  </TextAnimate>
                </div>
